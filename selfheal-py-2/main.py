...
def main():
    ...
    user_input = """Can you write a program that takes YAML input data and outputs it in a specific table format? 
    - The program should be able to handle any number of goals and subgoals, any number and granularity of periods, and any number of levels of or kinds of granularity (including periods like `weeks` or `decades`).
    ...
    - secondary:
      ...
      subgoals:
        - 2020-2025:
          ...
          subgoals:
            - 2023:
                goal: move in to jiuli home
                level: year
                state: 4
              subgoals:
                - Q1:
                    goal: basic furniture and setup
                    level: quarter
                    state: 3
    - secondary:
        goal: kin
      subgoals:
        - 2020-2025:
          ...
          subgoals:
            - 2023:
                goal: send latest letter
                level: year
                state: 3
              subgoals:
                - Q1:
                    goal: draft the letter
                    level: quarter
                    state: 2
```

Expected table output for the example YAML data above should be equivalent to `df` in the following:
```
import pandas as pd

data = {'Secondary': ['family', 'kin'], 
        '2020-2025': ['jiuli home,3', 'annual letters,3'], 
        '2023': ['move in to jiuli home,4', 'send latest letter,3'], 
        'Q1': ['basic furniture and setup,3', 'draft the letter,2']}
